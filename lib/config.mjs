...
export const CONFIG_DIR = join(homedir(), ".config", "s3pull");
...
export async function loadConfig(manifestConfig = {}) {
  const env = fromEnv();
  const file = await fromFile();
  const merged = { ...file, ...manifestConfig, ...env };
  ...
  if (missing.length > 0) {
    ...
    throw new Error(
      `Missing required config: ${missing.join(", ")}\n\n` +
        ...
        `Or add a config section to s3pull.yml:\n` +
        `  config:\n` +
        `    endpoint: https://...\n` +
        `    bucket: my-bucket\n\n` +
